...
// "api" for SOQL access, "refresh_token" so the connection survives past the
// first session without re-auth.
...
export async function GET(req: Request) {
  ...
  // Optional target workspace — used when an admin connects Salesforce on a
  ...
  if (!clientId) {
    return NextResponse.json(
      { error: "SALESFORCE_OAUTH_CLIENT_ID not configured" },
      { status: 500 }
    );
  }
  ...
  const redirectUri =
    ...
    `${new URL(req.url).origin}/api/oauth/salesforce/callback`;
  ...
  authUrl.searchParams.set("scope", SALESFORCE_SCOPES);
